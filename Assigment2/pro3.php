...
<body>
    ...
    $mang_duy_nhat = array();
    $so_lan = array();
    if(isset($POST['nhap_mang'])){
        $mang_so= explode(",", $_POST['nhap_mang']);
        $mang_duy_nhat = array_unique($mang_so);//xóa phần tử trùng lặp 
        $so_lan = array_count_values($mang_so);//đếm số lần xuất hiện 
        $chuoi="";
        foreach($so_lan as $key => $vaule){
            $chuoi = $chuoi.$key.":".$vaule."";
        }
